...
MAIN_MENU() {
  ...
  echo "$AVAILABLE_SERVICES" | while read SERVICE_ID BAR NAME 
    ...
    then 
      ...
      SERV_AVAIL=$($PSQL "SELECT service_id FROM services WHERE service_id = $SERVICE_ID_SELECTED ORDER BY service_id")
      ...
      then 
        ...
        read CUSTOMER_PHONE
        CUSTOMER_NAME=$($PSQL "SELECT name customer_id FROM customers WHERE phone = '$CUSTOMER_PHONE'")
        if [[ -z $CUSTOMER_NAME ]]
        then 
          echo -e "\nwhat's your name?"
          read CUSTOMER_NAME
        ...
        then 
          ...
          then 
            echo "I have put you down for a $NAME_SERV at $SERVICE_TIME, $(echo $CUSTOMER_NAME | sed -r 's/^ *| *$//g')."
          fi
        fi
      fi
    fi 
